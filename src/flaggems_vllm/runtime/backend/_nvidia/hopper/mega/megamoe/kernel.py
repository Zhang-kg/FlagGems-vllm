    ...
    if sm_idx == 0:
        if FAST_NVLINK_BARRIER:
            ...
            for pe in tl.static_range(0, NPES):
                tl.atomic_add(
                    peer_i32(sig_tab, pe) + slot, 1, sem="release", scope="sys"
                )
...
def nvlink_barrier(
    ...
    if sm_idx == 0:
        ...
        pe = tl.arange(0, 8)
        tl.atomic_add(
            peer_i32(sig_tab, pe) + slot, 1, mask=pe < NPES, sem="release", scope="sys"
        )
        sl = peer_i32(sig_tab, MY_PE)
        v = tl.atomic_add(sl + slot, 0, sem="acquire", scope="sys")
        while v < NPES:
...
def dispatch_dual_d8_pull(
    ...
    tle_raw.call(
        d8_tma1d_edsl, [stage0, state0, init_addr, d8_bytes, inactive, d8_init_op]
    )
    tle_raw.call(
        d8_tma1d_edsl, [stage1, state1, init_addr, d8_bytes, inactive, d8_init_op]
    )
    dispatch_sync(USE_D8_TMA1D)
    tle_raw.call(
        d8_tma1d_edsl, [stage0, state0, init_addr, d8_bytes, inactive, d8_fence_op]
    )
    tle_raw.call(
        d8_tma1d_edsl, [stage1, state1, init_addr, d8_bytes, inactive, d8_fence_op]
    )
    dispatch_sync(USE_D8_TMA1D)
    ...
    while tl.sum((done == 0).to(tl.int32), axis=0) > 0:
        ...
        while tl.sum(need.to(tl.int32), axis=0) > 0:  # D6, two cursors
            ...
            n = (tl.load(rsum_l + safe_cur, mask=need & ~ex, other=0) & 0xFFFFFFFF).to(
                tl.int32
            )
        ...
        while tl.sum((found == 0).to(tl.int32), axis=0) > 0:  # D7, two routes
            ...
            remaining = tl.where(
                hit[:, None],
                remaining,
                remaining - tl.minimum(remaining, length[:, None]),
            )
        ...
        tle_raw.call(
            ...
            [
                stage0,
                state0,
                _pick_stream2(src_addr, 0),
                d8_bytes,
                _pick_stream2(active_i32, 0),
                d8_load_op,
            ],
        )
        tle_raw.call(
            ...
            [
                stage1,
                state1,
                _pick_stream2(src_addr, 1),
                d8_bytes,
                _pick_stream2(active_i32, 1),
                d8_load_op,
            ],
        )
        dispatch_sync(USE_D8_TMA1D)
        ...
        sfv = tl.load(
            sf_base[:, None] + src_tok[:, None] * NSF + offs_sf[None, :],
            mask=live[:, None] & sf_ok[None, :],
            other=0.0,
        )
        tl.store(
            l1_sf + offs_sf[None, :] * POOL_TOKENS + pt[:, None],
            sfv,
            mask=live[:, None] & sf_ok[None, :],
        )
        ...
        tl.atomic_add(
            l1_arrival_count + (pool_blk + tok_in_e // BLOCK_M),
            1,
            mask=live,
            sem="release",
        )
...
def _resolve_one_d8_route(
    ...
    remaining = tl.where(
        rank_ok & live,
        tl.load(recv_l + rank_v * EPR + safe_e, mask=rank_ok, other=0).to(tl.int32),
        0,
    )
    slot = tok_in_e
    offset = 0
...
def dispatch_dual_d8_pull_scalar(
    ...
    init_addr = tl.cast(l1_token, tl.int64)
    tle_raw.call(
        d8_tma1d_edsl, [stage0, state0, init_addr, d8_bytes, inactive, init_op]
    )
    tle_raw.call(
        d8_tma1d_edsl, [stage1, state1, init_addr, d8_bytes, inactive, init_op]
    )
    dispatch_sync(USE_D8_TMA1D)
    tle_raw.call(
        d8_tma1d_edsl, [stage0, state0, init_addr, d8_bytes, inactive, fence_op]
    )
    tle_raw.call(
        d8_tma1d_edsl, [stage1, state1, init_addr, d8_bytes, inactive, fence_op]
    )
    dispatch_sync(USE_D8_TMA1D)
    ...
    while (done0 == 0) | (done1 == 0):
        ...
        tle_raw.call(
            d8_tma1d_edsl, [stage0, state0, src_addr0, d8_bytes, active0, load_op]
        )
        tle_raw.call(
            d8_tma1d_edsl, [stage1, state1, src_addr1, d8_bytes, active1, load_op]
        )
        dispatch_sync(USE_D8_TMA1D)

        sfv0 = tl.load(
            peer_f32(sf_tab, rank0) + src_tok0 * NSF + offs_sf,
            mask=live0 & sf_ok,
            other=0.0,
        )
        sfv1 = tl.load(
            peer_f32(sf_tab, rank1) + src_tok1 * NSF + offs_sf,
            mask=live1 & sf_ok,
            other=0.0,
        )
        ...
        tle_raw.call(
            d8_tma1d_edsl, [stage0, state0, dst_addr0, d8_bytes, active0, store_op]
        )
        tle_raw.call(
            d8_tma1d_edsl, [stage1, state1, dst_addr1, d8_bytes, active1, store_op]
        )
        dispatch_sync(USE_D8_TMA1D)
        ...
        tl.atomic_add(
            l1_arrival_count + (pool0 + tie0 // BLOCK_M), 1, mask=live0, sem="release"
        )
        tl.atomic_add(
            l1_arrival_count + (pool1 + tie1 // BLOCK_M), 1, mask=live1, sem="release"
        )
...
def dispatch_d8_pingpong_pull(
    ...
    while (done == 0) | (pending != 0):
        ...
        dispatch_sync(USE_D8_TMA1D)
        sfv = tl.load(
            peer_f32(sf_tab, src_rank) + src_tok * NSF + offs_sf,
            mask=live & sf_ok,
            other=0.0,
        )
        ...
        tle_raw.call(
            d8_tma1d_edsl,
            [pending_stage, pending_state, pending_dst, d8_bytes, pending, store_op],
        )
        dispatch_sync(USE_D8_TMA1D)
        ...
        tl.atomic_add(
            l1_arrival_count + (pending_pool + pending_tie // BLOCK_M),
            1,
            mask=pending != 0,
            sem="release",
        )
...
def dispatch_role(
    ...
    for it in tl.static_range(0, ITERS):  # D1 count
        ...
        if USE_SMEM_EXPERT_COUNT:
            tl.atomic_add(
                tle.gpu.local_ptr(ec_s, (tl.where(ok1, e, 0),)),
                1,
                mask=ok1,
                sem="relaxed",
                scope="cta",
    ...
    old = tl.atomic_add(
        expert_send_count + offs_e, send_value, mask=e_ok, sem="relaxed"
    )
    if USE_SMEM_EXPERT_COUNT:
    ...
    for it in tl.static_range(0, ITERS):  # D3 -> OWNER's queue
        ...
        if USE_SMEM_EXPERT_COUNT:
            d3_slot = tl.atomic_add(
                tle.gpu.local_ptr(ec_s, (tl.where(valid, e, 0),)),
                1,
                mask=valid,
                sem="relaxed",
                scope="cta",
    ...
    if sm_idx == 0:  # D4 -> every owner
        ...
        tl.store(
            peer_i64(recv_tab, d4_owner) + (MY_PE * EPR + d4_le),
            status & 0xFFFFFFFF,
            mask=e_ok,
        )
        tl.atomic_add(
            peer_i64(rsum_tab, d4_owner) + d4_le,
            status,
            mask=e_ok,
            sem="relaxed",
            scope="sys",
    ...
    while done == 0:
        ...
        while found == 0:
            ...
            offset = tl.where(hit, offset, offset + length)
            remaining = tl.where(
                hit, remaining, remaining - tl.minimum(remaining, length)
            )
            found = tl.where(hit, 1, 0)
...
def loader_role(
    ...
    while stop == 0:
            ...
            if cur_e >= EPR:
                ...
                if phase == 1:
                    ...
                    while (found == 0) & (cur_e < wave_end):
                        expert_status = tl.atomic_add(
                            rsum_l + cur_e, 0, sem="acquire", scope="sys"
                        )
        ...
        if stop == 0:
            ...
            if phase == 1:
                if LOAD_A:
                    l1_arrival = tl.atomic_add(
                        l1_arrival_count + pool_block, 0, sem="acquire"
                    )
                ...
                if LOAD_A:
                    l2_arrival = tl.atomic_add(
                        l2_arrival_mask + pool_block, 0, sem="acquire"
                    )
...
def math_role(
    ...
    if MATH_RSUM_EARLY:
        ...
        ready_e = 0
        while ready_e < EPR:
            ready_status = tl.atomic_add(
                rsum_l + ready_e, 0, sem="acquire", scope="sys"
            )
            ...
            for kb in tl.range(0, nk):
                ...
                if PREFETCH_SFB:
                    ...
                    if phase == 1:
                        ...
                        sfb_u = tl.load(
                            w1_sf + (cur_e * NL1N + NL1N // 2 + _gn) * NK1 + kb
                        )
                    else:
                ...
                if phase == 1:
                    ...
                    acc_lo = tle.gpu.wgmma(
                        sl.a.subslice(row_base, WG_M, 0),
                        sl.b,
                    ...
                    sf_lo = tl.reshape(
                        tl.load(
                            tle.gpu.local_ptr(
                                sl.sfa.slot(0).subslice(row_base, WG_M, 0)
                            )
                        ),
                        (WG_M,),
                    )
                    ...
                    acc_lo = tle.gpu.wgmma(
                        a_my.subslice(0, HK, -1),
                        sl.b.subslice(0, HK, -1),
                        out_dtype=tl.float32,
                        trans_b=True,
                    )
                    sf_lo = tl.reshape(
                    ...
                    acc_hi = tle.gpu.wgmma(
                        a_my.subslice(HK, HK, -1),
                        sl.b.subslice(HK, HK, -1),
                        out_dtype=tl.float32,
                        trans_b=True,
                    )
                    sf_hi = tl.reshape(
                        tl.load(
                            tle.gpu.local_ptr(
                                sl.sfa.slot(1).subslice(row_base, WG_M, 0)
                            )
                        ),
                        (WG_M,),
                    )
                ...
                if PREFETCH_SFB & (kb + 1 < nk):
                    if phase == 1:
                        _gn = n_block // 2
                        sfb_g_next = tl.load(
                            w1_sf + (cur_e * NL1N + _gn) * NK1 + kb + 1
                        )
                        sfb_u_next = tl.load(
                            w1_sf + (cur_e * NL1N + NL1N // 2 + _gn) * NK1 + kb + 1
                        )
                    else:
                        sfb_g_next = tl.load(
                            w2_sf + (cur_e * NL2N + n_block) * NK2 + kb + 1
                        )
                        sfb_u_next = sfb_g_next
            ...
            if phase == 1:
                if do_epilogue:
                    ...
                    if USE_L1_STORE_PIPE:
                        tl.store(
                            tle.gpu.local_ptr(l1o_s.slot(WG)), q, mask=m_ok[:, None]
                        )
                        tle.gpu.copy(
                            l1o_s.slot(WG),
                            l1_st_desc,
                        ...
                        tl.store(
                            l2_acts + rows[:, None] * INTER + cols_o[None, :],
                            q,
                            mask=m_ok[:, None],
                        )
                ...
                tl.debug_barrier()
                tl.atomic_or(
                    l2_arrival_mask + pool_block,
                    (tl.full((), 1, tl.int64) << (n_block + WG * 32)),
                    sem="release",
                )
            else:
                if do_epilogue:
                    ...
                    if WRITE_L2_OUT:
                        tl.store(
                            l2o_l + rows[:, None] * L2_N + cols[None, :],
                            acc,
                            mask=m_ok[:, None],
                        )
                    md = meta_l + rows * 3
                    ...
                    cd_addr = smem_generic_addr(
                        tle.gpu.local_ptr(cd_s.slot(WG), (0, 0))
                    )
                    dst_addr = smem_generic_addr(
                        tle.gpu.local_ptr(dst_rows_s.slot(WG), (0,))
                    )
                    valid_m_wg = tl.maximum(tl.minimum(valid_m - row_base, WG_M), 0)
                    bar_wg = tl.full(
                        (), 9 + WG, tl.int32
                    )  # per-WG named barrier (9 / 10)
...
def ws_megakernel(
    ...
    a_s = tle.gpu.alloc(
        [STAGES, BLOCK_M, BLOCK_K], dtype=tl.float8e4nv, layout=None, scope=tle.gpu.smem
    )
    b_s = tle.gpu.alloc(
        [STAGES, BLOCK_N, BLOCK_K], dtype=tl.float8e4nv, layout=None, scope=tle.gpu.smem
    )
    sfa_s = tle.gpu.alloc(
        [STAGES, 2, BLOCK_M], dtype=tl.float32, layout=None, scope=tle.gpu.smem
    )
    cd_s = tle.gpu.alloc(
        [2, BLOCK_M // 2, BLOCK_N],
        dtype=tl.bfloat16,
    ...
    l1o_s = tle.gpu.alloc(
        [2, BLOCK_M // 2, L1_OUT_N],
        dtype=tl.float8e4nv,
        layout=None,
        scope=tle.gpu.smem,
    )

    d8_token0_s = tle.gpu.alloc(
        [K],
        dtype=tl.float8e4nv,
        layout=None,
        scope=tle.gpu.smem,
...
def run_worker() -> int:
    ...
    if shared_data_dir:
        ...
        data_label = (
            f"{dataset['manifest']['source_model']}@{dataset['manifest']['source_revision']}"
        ...
        w1_sf = (
            torch.rand(EPR, NL1N, NK1, generator=gg, device=dev) * 0.4 + 0.3
        ).contiguous()
        w2_q = (torch.randn(EPR, L2_N, INTER, generator=gg, device=dev) * 0.4).to(f8)
        w2_sf = (
            torch.rand(EPR, NL2N, NK2, generator=gg, device=dev) * 0.4 + 0.3
        ).contiguous()
    if tuple(w1_q.shape) != (EPR, L1_N, K):
        raise ValueError(
            f"invalid W1 weight shape {tuple(w1_q.shape)}; expected {(EPR, L1_N, K)}"
        )
    ...
    if tuple(w1_sf.shape) != (EPR, NL1N, NK1):
        raise ValueError(
            f"invalid W1 SF shape {tuple(w1_sf.shape)}; expected {(EPR, NL1N, NK1)}"
        )
    ...
    def peers(base):
        ...
        for pe in range(R):
            p = (
                base
                if pe == rank
                else lib.nvshmem_ptr_wrapper(ctypes.c_void_p(base), pe)
            )
            if not p:
    ...
    cb_tab, sig_tab, meta_tab, l2out_tab = (
        peers(cb_b),
        peers(sig_b),
        peers(meta_b),
        peers(l2out_b),
    ...
    for tab, nb in (
        (q_tab, EPR * R * MAX_RECV * 4),
        (recv_tab, R * EPR * 8),
        (rsum_tab, EPR * 8),
        (cb_tab, TOPK * NTOK * K * 2),
        (sig_tab, 16 * 4),
        (meta_tab, POOL_TOKENS * 3 * 4),
    ...
    a1_desc = TensorDescriptor(
        l1_tok, shape=[POOL_TOKENS, K], strides=[K, 1], block_shape=[BLOCK_M, BLOCK_K]
    )
    # v4: fp8 wgmma wants B N-major, which is w1_q's NATIVE layout -- no permute.
    w1_nk = w1_q.contiguous()  # [EPR, L1_N, K]  N-major
    b1_desc = TensorDescriptor(
        w1_nk.view(EPR * L1_N, K),
        shape=[EPR * L1_N, K],
        strides=[K, 1],
        block_shape=[BLOCK_N, BLOCK_K],
    )
    a2_desc = TensorDescriptor(
        l2_acts,
        shape=[POOL_TOKENS, INTER],
        strides=[INTER, 1],
        block_shape=[BLOCK_M, BLOCK_K],
    )
    l1_st_desc = TensorDescriptor(
        l2_acts,
        shape=[POOL_TOKENS, INTER],
        strides=[INTER, 1],
        block_shape=[BLOCK_M // 2, BLOCK_N // 2],
    )  # v32: L1 async TMA store
    w2_nk = w2_q.contiguous()  # [EPR, L2_N, INTER]  N-major
    b2_desc = TensorDescriptor(
        w2_nk.view(EPR * L2_N, INTER),
        shape=[EPR * L2_N, INTER],
        strides=[INTER, 1],
        block_shape=[BLOCK_N, BLOCK_K],
    )
    # SF is MN-major -> a block's SF tile is BLOCK_M contiguous floats: a real TMA box
    sfa1_desc = TensorDescriptor(
        l1_sf.view(NSF, POOL_TOKENS),
        shape=[NSF, POOL_TOKENS],
        strides=[POOL_TOKENS, 1],
        block_shape=[1, BLOCK_M],
    )
    sfa2_desc = TensorDescriptor(
        l2_sf.view(NL1N, POOL_TOKENS),
        shape=[NL1N, POOL_TOKENS],
        strides=[POOL_TOKENS, 1],
    ...
    if "EARLY_PIPE_RELEASE" in kernel_arg_names:
        kargs["EARLY_PIPE_RELEASE"] = bool(
            getattr(EARLY_PIPE_RELEASE, "value", EARLY_PIPE_RELEASE)
        )
    ...
    try:
        ...
        print(
            ...
            + repr(c)[:300]
            + " FILENAME="
            + str(getattr(c, "filename", None))
            + " args="
            + str(getattr(c, "args", None)),
            flush=True,
        )
        raise
    barrier()
    ...
    if baseline_kernel is not None:
        ...
        for _name in [
            *(f"sp{i}" for i in range(8)),
            *(f"wp{i}" for i in range(8)),
            "MATH_PREFETCH_SFB",
        ]:
    ...
    if BENCH:
        ...
        if baseline_us is not None:
            ...
            matched = f" paired_base={baseline_us:.1f}us " f"speedup={speedup:.4f}x"
        print(
    ...
    cb_c = (
        d2h(cb_tab, rank, TOPK * NTOK * K * 2)
        .cpu()
        .view(torch.bfloat16)
        .view(TOPK, NTOK, K)
    )
    peer_meta = [
        d2h(meta_tab, pe, POOL_TOKENS * 3 * 4)
        .cpu()
        .view(torch.int32)
        .view(POOL_TOKENS, 3)
        for pe in range(R)
    ...
    peer_l2out = [
        d2h(l2out_tab, pe, POOL_TOKENS * L2_N * 4)
        .cpu()
        .view(torch.float32)
        .view(POOL_TOKENS, L2_N)
        for pe in range(R)
    ]
    torch.cuda.synchronize()
    ...
    for le in range(EPR):
        for s in range(R):
            ...
            got = sorted(
                int(v) for v in q_c[base : base + counts[rank][le][s]].tolist()
            )
    ...
    for le in range(EPR):
        ...
        for b in range(blocks[rank][le]):
            if int(arr_c[pool_off[rank][le] + b]) != min(
                BLOCK_M, n_le[rank][le] - b * BLOCK_M
            ):
                bump("l1_arrival")

    if (
        int(loader_blocks.cpu().sum()) != EXPECT
        or int(math_blocks.cpu().sum()) != EXPECT
    ):
        bump("block_coverage")
    ...
    for nb in range(NL2N):
        for kb in range(NK2):
            W2loc[
                :, nb * BLOCK_N : (nb + 1) * BLOCK_N, kb * BLOCK_K : (kb + 1) * BLOCK_K
            ] *= w2sf_c[:, nb, kb][:, None, None]
